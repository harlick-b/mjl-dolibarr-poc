...
function cleanupActivityExecution() {
  sql(`
    SET @actexec_activities = (SELECT GROUP_CONCAT(rowid) FROM llx_mjlfinancement_activity WHERE ref LIKE 'ACTX-%');
    SET @actexec_projects = (SELECT GROUP_CONCAT(rowid) FROM llx_projet WHERE ref LIKE 'ACTX-%');
    DELETE FROM llx_mjlfinancement_workflow_action WHERE (object_type = 'mjlfinancement_activity' AND FIND_IN_SET(object_id, COALESCE(@actexec_activities, ''))) OR (object_type = 'mjlfinancement_project' AND FIND_IN_SET(object_id, COALESCE(@actexec_projects, '')));
    DELETE FROM llx_mjlfinancement_activity WHERE ref LIKE 'ACTX-%';
    DELETE FROM llx_projet WHERE ref LIKE 'ACTX-%';
  `);
}

function seedActivityExecution() {
  sql(`
    ...
    VALUES
      (1, 'ACTX-EXEC', 'Activite Activity execution execution', @project, @convention, '2026-06-20', '2026-07-20', NOW(), @agent, 'ACTXEXEC', 0),
      (1, 'ACTX-LATE', 'Activite Activity execution en retard', @project, @convention, '2026-06-20', '2026-07-01', NOW(), @agent, 'ACTXLATE', 0),
      (1, 'ACTX-SELF', 'Activite Activity execution no self', @project, @convention, '2026-06-20', '2026-07-15', NOW(), @verifier, 'ACTXSELF', 3);
  `);
}

test.beforeAll(() => {
  cleanupActivityExecution();
  seedActivityExecution();
});
...
test('activity options are scoped and mismatched project/convention POST is rejected', async ({ page }) => {
  ...
  const response = await page.request.post('/custom/mjlfinancement/activities.php', {
    form: { token, action: 'create', ref: 'ACTX-TAMPER', label: 'Activite Activity execution incoherente', fk_project: projectId, fk_convention: redConvention },
    maxRedirects: 0,
  });
  expect([302, 403]).toContain(response.status());
  expect(Number(scalar("SELECT COUNT(*) FROM llx_mjlfinancement_activity WHERE ref = 'ACTX-TAMPER' AND entity = 1"))).toBe(0);
});

test('update_execution updates only execution fields and writes production audit role', async ({ page }) => {
  const activityId = scalar("SELECT rowid FROM llx_mjlfinancement_activity WHERE ref = 'ACTX-EXEC' AND entity = 1 LIMIT 1");
...
test('project and dashboard execution KPIs reflect execution update and late alert is visible', async ({ page }) => {
  const projectId = scalar("SELECT rowid FROM llx_projet WHERE ref = 'PRJ-JE-2026' AND entity = 1 LIMIT 1");
...
test('no-self-prevalidation still holds after execution work', async ({ page }) => {
  const activityId = scalar("SELECT rowid FROM llx_mjlfinancement_activity WHERE ref = 'ACTX-SELF' AND entity = 1 LIMIT 1");
  await login(page, 'superviseur.n1');
  const response = await postActivity(page, activityId, { action: 'prevalidate', comment: 'Tentative no self Activity execution' });
  expect([302, 403]).toContain(response.status());
