...
function cleanupConventionManagementFixtures() {
  sql(`
    SET @convention_management_conventions = (SELECT GROUP_CONCAT(rowid) FROM llx_mjlfinancement_convention WHERE ref LIKE 'CNV-%');
    DELETE FROM llx_mjlfinancement_workflow_action WHERE object_type = 'mjlfinancement_convention' AND FIND_IN_SET(object_id, COALESCE(@convention_management_conventions, ''));
    DELETE FROM llx_mjlfinancement_activity WHERE ref LIKE 'CNV-%';
    DELETE FROM llx_mjlfinancement_expense WHERE ref LIKE 'CNV-%';
    DELETE FROM llx_mjlfinancement_convention WHERE ref LIKE 'CNV-%';
  `);
}

test.beforeAll(() => {
  cleanupConventionManagementFixtures();
});
...
test('Current final-validator fixture receives convention write without routine operation rights', async () => {
...
test('Current final-validator fixture creates, edits, activates, closes, and views convention history', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /CNV-UI-CONV/ })).toBeVisible();
  ...
  await expect(page.getByText('Correction libelle Convention management')).toBeVisible();
  ...
  await page.getByLabel('Motif de clôture').fill('Cloture test Convention management');
  await page.getByRole('button', { name: 'Clôturer l’enveloppe' }).click();
  await expect(page.getByText('Clôturée').first()).toBeVisible();
  await expect(page.getByText('Cloture test Convention management')).toBeVisible();

  await page.goto('/custom/mjlfinancement/reports.php?report=convention_budget');
  await expect(page.locator('select[name="convention_id"] option', { hasText: 'CNV-UI-CONV' })).toHaveCount(1);
});

test('Admin can open convention management, while unauthorized direct access and POST are blocked', async ({ page }) => {
  ...
  const before = Number(scalar("SELECT COUNT(*) FROM llx_mjlfinancement_convention WHERE ref = 'CNV-FORBIDDEN' AND entity = 1"));
  ...
  expect(Number(scalar("SELECT COUNT(*) FROM llx_mjlfinancement_convention WHERE ref = 'CNV-FORBIDDEN' AND entity = 1"))).toBe(before);
});
...
test('Linked conventions reject locked-field edits and store sanitized unsafe history', async ({ page }) => {
  ...
  const response = await page.request.post(`/custom/mjlfinancement/conventions.php?id=${conventionId}`, {
    form: {
      ...
      note_private: '',
      comment: 'Tentative verrouillee Convention management'
    },
    maxRedirects: 0
  });
  ...
  expect(Number(scalar(`
    ...
    AND changes_json NOT LIKE '%CNV-TAMPER-LOCKED%'
  `))).toBeGreaterThan(0);
});
...
test('Only active conventions can be selected or posted for new activities and expenses', async ({ page }) => {
  ...
  expect(Number(scalar("SELECT COUNT(*) FROM llx_mjlfinancement_activity WHERE ref = 'CNV-DRAFT-CONV-ACT' AND entity = 1"))).toBe(0);
  ...
  expect([302, 403]).toContain(response.status());
  expect(Number(scalar("SELECT COUNT(*) FROM llx_mjlfinancement_expense WHERE ref = 'CNV-DRAFT-CONV-EXP' AND entity = 1"))).toBe(0);
});
