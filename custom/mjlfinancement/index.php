...
if ($capabilities['admin']) {
	mjl_dashboard_render_card_section(
		...
		array(
			...
			array('label' => 'Risques echeance', 'value' => $dashboardMetrics['deadline_risks'], 'context' => 'Activites ouvertes a traiter avant ou apres echeance', 'href' => '/custom/mjlfinancement/alerts.php', 'action' => 'Ouvrir les alertes', 'status' => 'Supervision', 'tone' => $dashboardMetrics['deadline_risks'] > 0 ? 'warning' : 'neutral'),
...
if (!$capabilities['admin'] && $capabilities['operational']) {
	mjl_dashboard_render_card_section(
		...
		array(
			...
			array('label' => 'Alertes actives', 'value' => $alertCount, 'context' => 'Delais et justificatifs incomplets dans votre perimetre', 'href' => '/custom/mjlfinancement/alerts.php', 'action' => 'Ouvrir les alertes', 'status' => 'Action attendue', 'tone' => $alertCount > 0 ? 'warning' : 'neutral'),
		)
	);
}
...
if (!$capabilities['admin'] && $capabilities['reviewer']) {
	mjl_dashboard_render_card_section(
		...
		array(
			...
			array('label' => 'Risques echeance', 'value' => $dashboardMetrics['deadline_risks'], 'context' => 'Activites ouvertes a verifier avant ou apres echeance', 'href' => '/custom/mjlfinancement/alerts.php', 'action' => 'Ouvrir les alertes', 'status' => 'Delai', 'tone' => $dashboardMetrics['deadline_risks'] > 0 ? 'warning' : 'neutral'),
			array('label' => 'Alertes actives', 'value' => $alertCount, 'context' => 'Risques et decisions attendues dans la file', 'href' => '/custom/mjlfinancement/alerts.php', 'action' => 'Ouvrir les alertes', 'status' => 'Alertes', 'tone' => $alertCount > 0 ? 'warning' : 'neutral'),
		)
	);
}
...
if (!$capabilities['admin'] && $capabilities['supervision']) {
	mjl_dashboard_render_card_section(
		...
		array(
			...
			array('label' => 'Risques echeance', 'value' => $dashboardMetrics['deadline_risks'], 'context' => 'Activites ouvertes avec delai proche ou depasse', 'href' => '/custom/mjlfinancement/alerts.php', 'action' => 'Analyser les risques', 'status' => 'Delai', 'tone' => $dashboardMetrics['deadline_risks'] > 0 ? 'warning' : 'neutral'),
			array('label' => 'Rapports disponibles', 'value' => $metrics['reports_available'], 'context' => 'Exports et rapports MJL', 'href' => '/custom/mjlfinancement/reports.php', 'action' => 'Ouvrir les rapports', 'status' => 'Sorties officielles', 'tone' => 'neutral'),
		)
	);
}
...
function mjl_workspace_render_navigation($capabilities)
{
	...
	if ($capabilities['activity_read'] || $capabilities['expense_read']) {
		$items[] = array('Alertes', '/custom/mjlfinancement/alerts.php', 'Risques, decisions attendues et justificatifs incomplets');
	}
