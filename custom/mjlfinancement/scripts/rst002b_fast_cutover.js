...
function run(args, options = {}) {
  ...
  if (result.error || result.status !== 0) throw new Error(`${options.label || 'Docker command'} failed.${result.stderr ? ` ${result.stderr.trim()}` : ''}`);
  return result.stdout || '';
}
function dc(args, options) { return run([...compose, ...args], options); }
function migration(mode) {
  dc(['run', '--rm', '--no-deps', '-e', 'MJL_RST002B_SIMPLE_CUTOVER=1', '-e', `MJL_RST002B_SIMPLE_PROJECT=${project}`, '--entrypoint', '/usr/local/bin/php', 'dolibarr', '/var/www/html/custom/mjlfinancement/scripts/rst002b_activity_assignment.php', `--mode=${mode}`, '--confirm=RST-002B'], { label: `RST-002B ${mode}` });
}
