...
$selfCorrectionRequest = $activity->requestCorrection($agent, 'Self correction request should fail', 'AGENT_VERIFICATEUR', 1);
if ($selfCorrectionRequest >= 0) {
	fail('Self-correction request unexpectedly succeeded.');
}

$selfRejection = $activity->reject($agent, 'Self rejection should fail', 'AGENT_VERIFICATEUR', 1);
if ($selfRejection >= 0) {
