...
$validationId = insertRow('mjlfinancement_validation', array(
	'entity' => $entity,
	'ref' => 'MJL-SCOPE-VAL-'.$suffix,
	'fk_expense' => $expenseId,
	'action' => 'submitted',
	'from_status' => 'draft',
	'to_status' => 'submitted',
	'fk_user_action' => $adminUser->id,
	'actor_role' => 'ADMIN_PLATEFORME',
	'action_date' => $db->idate(dol_now()),
	'comment' => 'Scope verification validation',
	'date_creation' => $db->idate(dol_now()),
	'fk_user_creat' => $adminUser->id,
	'import_key' => $importKey,
));
...
$scopeQueryFamilies = array(
	'projects.php / project detail and lists' => array('type' => 'mjlfinancement_project', 'id' => $projectId),
	'conventions.php / convention queries' => array('type' => 'mjlfinancement_convention', 'id' => $conventionId),
	'fundreceipts.php / receipt queries' => array('type' => 'mjlfinancement_fund_receipt', 'id' => $fundReceiptId),
	'activities.php / activity queries' => array('type' => 'mjlfinancement_activity', 'id' => $activityId),
	'expenses.php / expense queries' => array('type' => 'mjlfinancement_expense', 'id' => $expenseId),
	'budgetlines.php / budget queries' => array('type' => 'mjlfinancement_budget_line', 'id' => $budgetLineId),
	'validations.php / decision-history queries' => array('type' => 'mjlfinancement_validation', 'id' => $validationId),
	'workflowactions.php / audit queries' => array('type' => 'mjlfinancement_workflow_action', 'id' => $workflowId),
	'exchanges.php / contextual-exchange queries' => array('type' => 'mjlfinancement_exchange_log', 'id' => $exchangeLogId),
	'document routes / ECM queries' => array('type' => 'ecm_files', 'id' => $documentId),
	'project notes / timeline queries' => array('type' => 'mjlfinancement_project_note', 'id' => $projectNoteId),
);
foreach ($scopeQueryFamilies as $family => $target) {
	$type = $target['type'];
	$id = $target['id'];
	assertSame($thirdpartyId, mjl_scope_object_fk_soc($type, $id, $entity), $family.' should resolve to the scoped partner.');
	assertTrue(mjl_scope_can_access_object($scopedUser, $type, $id, $entity), $family.' should be accessible for assigned scope.');
	assertFalse(mjl_scope_can_access_object($unresolvedUser, $type, $id, $entity), $family.' should fail closed for unresolved non-admin.');
	assertSame(null, mjl_scope_object_fk_soc($type, $id, $entity + 1), $family.' should not resolve across entities.');
	assertFalse(mjl_scope_can_access_object($scopedUser, $type, $id, $entity + 1), $family.' should fail closed across entities.');
}
...
cleanup($importKey);
out('MJL scope access model verification: OK');
...
function cleanup($importKey)
{
	...
	$queries = array(
		...
		'DELETE FROM '.$db->prefix()."mjlfinancement_validation WHERE import_key = '".$escaped."'",
