...
try {
	...
	queryTrace("INSERT INTO {$db->prefix()}projet (entity,ref,title,fk_soc,fk_statut,datec,fk_user_creat) VALUES (1,'{$marker}-PROJECTA','Valid',{$partnerPrimary},1,NOW(),1),(2,'{$marker}-PROJECTB','Cross',{$partnerCross},1,NOW(),1),(1,'{$marker}-PC','Corrupt',{$partnerCross},1,NOW(),1),(1,'{$marker}-PALT','Same-entity alternate',{$partnerPrimary},1,NOW(),1)");
	...
	$projectAlternate=idTrace("SELECT rowid FROM {$db->prefix()}projet WHERE ref='{$marker}-PALT'");
	queryTrace("INSERT INTO {$db->prefix()}mjlfinancement_convention (entity,ref,title,fk_soc,fk_project,total_amount,currency_code,status,date_creation,fk_user_creat) VALUES (1,'{$marker}-C1','Valid',{$partnerPrimary},{$projectPrimary},1,'XOF',1,NOW(),1),(2,'{$marker}-C2','Cross',{$partnerCross},{$projectCross},1,'XOF',1,NOW(),1),(1,'{$marker}-CC','Corrupt',{$partnerPrimary},{$projectCross},1,'XOF',1,NOW(),1),(1,'{$marker}-CALT','Same-entity alternate',{$partnerPrimary},{$projectAlternate},1,'XOF',1,NOW(),1)");
	...
	$cAlternate=idTrace("SELECT rowid FROM {$db->prefix()}mjlfinancement_convention WHERE ref='{$marker}-CALT'");
	queryTrace("INSERT INTO {$db->prefix()}mjlfinancement_activity (entity,ref,label,fk_project,fk_convention,status,date_creation,fk_user_creat) VALUES (1,'{$marker}-A1','Valid',{$projectPrimary},{$c1},0,NOW(),1),(2,'{$marker}-A2','Cross',{$projectCross},{$c2},0,NOW(),1),(1,'{$marker}-AC','Corrupt',{$projectPrimary},{$cc},0,NOW(),1),(1,'{$marker}-AALT','Same-entity alternate',{$projectAlternate},{$cAlternate},0,NOW(),1)");
	...
	$aAlternate=idTrace("SELECT rowid FROM {$db->prefix()}mjlfinancement_activity WHERE ref='{$marker}-AALT'");
	queryTrace("INSERT INTO {$db->prefix()}mjlfinancement_budget_line (entity,ref,label,fk_project,fk_convention,initial_budget,date_creation,fk_user_creat,status) VALUES (1,'{$marker}-B1','Valid',{$projectPrimary},{$c1},1,NOW(),1,1),(2,'{$marker}-B2','Cross',{$projectCross},{$c2},1,NOW(),1,1),(1,'{$marker}-BC','Corrupt',{$projectPrimary},{$cc},1,NOW(),1,1),(1,'{$marker}-BALT','Same-entity alternate',{$projectAlternate},{$cAlternate},1,NOW(),1,1)");
	...
	$bAlternate=idTrace("SELECT rowid FROM {$db->prefix()}mjlfinancement_budget_line WHERE ref='{$marker}-BALT'");
	queryTrace("INSERT INTO {$db->prefix()}mjlfinancement_expense (entity,ref,fk_project,fk_convention,fk_budget_line,fk_mjl_activity,amount,date_creation,fk_user_creat,status) VALUES (1,'{$marker}-E1',{$projectPrimary},{$c1},{$b1},NULL,1,NOW(),1,0),(2,'{$marker}-E2',{$projectCross},{$c2},{$b2},NULL,1,NOW(),1,0),(1,'{$marker}-EC',{$projectPrimary},{$cc},{$bc},NULL,1,NOW(),1,0),(1,'{$marker}-EBM',{$projectPrimary},{$c1},{$bAlternate},NULL,1,NOW(),1,0),(1,'{$marker}-EAM',{$projectPrimary},{$c1},{$b1},{$aAlternate},1,NOW(),1,0)");
	...
	$expenseBudgetMismatch=idTrace("SELECT rowid FROM {$db->prefix()}mjlfinancement_expense WHERE ref='{$marker}-EBM'");
	$expenseActivityMismatch=idTrace("SELECT rowid FROM {$db->prefix()}mjlfinancement_expense WHERE ref='{$marker}-EAM'");
	$expenseIntegrity=mjl_traceability_expense_target_integrity_sql('v.rowid', 'v.entity');
	if (idTrace("SELECT COUNT(*) FROM {$db->prefix()}mjlfinancement_expense v WHERE v.rowid={$e1} AND {$expenseIntegrity}") !== 1) failTrace('Complete Expense predicate rejected the valid parent chain.');
	if (idTrace("SELECT COUNT(*) FROM {$db->prefix()}mjlfinancement_expense v WHERE v.rowid={$expenseBudgetMismatch} AND {$expenseIntegrity}") !== 0) failTrace('Complete Expense predicate admitted a same-entity Budget Line mismatch.');
	if (idTrace("SELECT COUNT(*) FROM {$db->prefix()}mjlfinancement_expense v WHERE v.rowid={$expenseActivityMismatch} AND {$expenseIntegrity}") !== 0) failTrace('Complete Expense predicate admitted a corrupt optional Activity link.');
