...
print '<tr class="liste_titre"><th>Ref</th><th>Depense</th><th>Action</th><th>De</th><th>Vers</th><th>Acteur</th><th>Date</th><th>Commentaire</th></tr>';
if ($resql) while ($obj = $db->fetch_object($resql)) {
	print '<tr class="oddeven"><td>'.dol_escape_htmltag($obj->ref).'</td><td>'.dol_escape_htmltag($obj->expense_ref).'</td><td>'.dol_escape_htmltag($obj->action).'</td><td>'.dol_escape_htmltag($obj->from_status).'</td><td>'.dol_escape_htmltag($obj->to_status).'</td><td>'.dol_escape_htmltag($obj->login).'</td><td>'.dol_escape_htmltag($obj->action_date).'</td><td>'.dol_escape_htmltag($obj->comment).'</td></tr>';
}
print '</table></div>';
