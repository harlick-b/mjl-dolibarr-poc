...
{
	global $user;
	foreach($rows as$row){$url=DOL_URL_ROOT.'/custom/mjlfinancement/activities.php?id='.(int)$row->activity_id;print '<article class="mjl-operation-card"><h2>'.dol_escape_htmltag($row->name).'</h2><p><a href="'.$url.'">'.dol_escape_htmltag($row->activity_ref.' - '.$row->activity_name).'</a></p><dl class="mjl-activity-meta"><div><dt>Partenaire</dt><dd>'.dol_escape_htmltag($row->partner_name).'</dd></div><div><dt>Projet</dt><dd>'.dol_escape_htmltag(trim($row->project_ref.' - '.$row->project_title)).'</dd></div><div><dt>Type d’Opération</dt><dd>'.dol_escape_htmltag($row->type_label).'</dd></div><div><dt>Statut</dt><dd>'.dol_escape_htmltag(mjl_operation_status_label($row->status)).'</dd></div><div><dt>Montant autorisé</dt><dd>'.dol_escape_htmltag(mjl_format_money($row->authorized_amount)).'</dd></div><div><dt>Montant dépensé</dt><dd>'.dol_escape_htmltag(mjl_format_money($row->spent_amount)).'</dd></div><div><dt>Écart</dt><dd>'.dol_escape_htmltag(mjl_execution_variance_amount($row->spent_amount,$row->authorized_amount)).'</dd></div><div><dt>Variance %</dt><dd>'.dol_escape_htmltag(mjl_execution_variance_percent($row->spent_amount,$row->authorized_amount)).'</dd></div></dl>';
		if($row->observation!==null&&$row->observation!=='')print '<p><strong>Observation :</strong> '.dol_escape_htmltag($row->observation).'</p>';
		$canAct=mjl_scope_is_input_agent($user)&&(string)$row->validation_status==='FINAL_VALIDATED'&&empty($row->is_cancelled);
