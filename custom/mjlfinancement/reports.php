...
function mjl_reports_project_options()
{
	...
	$sql = 'SELECT rowid, ref, title FROM '.$db->prefix().'projet WHERE entity = '.((int) $conf->entity).' AND 1=0'.' ORDER BY ref';
	$resql = $db->query($sql);
	$options = array();
...
function mjl_reports_convention_options()
{
	...
	$sql = 'SELECT rowid, ref, title FROM '.$db->prefix().'mjlfinancement_convention WHERE entity = '.((int) $conf->entity).' AND 1=0'.' ORDER BY ref';
	$resql = $db->query($sql);
	$options = array();
	if ($resql) while ($obj = $db->fetch_object($resql)) $options[(int) $obj->rowid] = mjl_reports_target_wording($obj->ref.' - '.$obj->title);
	return $options;
