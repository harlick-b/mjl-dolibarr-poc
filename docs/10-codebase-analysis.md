...
The current implementation is credible for a POC. It includes a custom data model, installation/bootstrap scripts, seed data, permissions, dashboard views, reports, and a working expense validation workflow. It is not yet a polished production application: several screens are basic table/form views, many operations are only available through scripts or object classes, and some documentation files are still empty.
...
- The custom module is `MjlFinancement`, version `0.3.0`.
...
The module defines seven custom database-backed objects:
...
Each object has a Dolibarr `CommonObject` class and matching SQL table definitions. The most complete domain behavior is implemented around expenses.
...
Upgrade scripts are present for schema versions `0.2.0` and `0.3.0`.
...
- The form uses raw numeric IDs for project, convention, activity, and budget line.
...
- Expense list with supporting document status.

Available report features:

- Filters for project, convention, status, date start, and date end.
- Table display.
- CSV export for users with export write permission.

Current limitations:

- Report filters use numeric IDs.
...
All seven custom classes expose basic Dolibarr object methods:
...
- Expense workflow is implemented, but the UI remains basic and ID-driven.
- Activities do not currently have a dedicated custom list page linked from the dashboard.
- No custom REST API endpoints were found.
- No polished navigation, detail pages, search, pagination controls, or selector widgets.
