...
- Guarded document downloads add best-effort `document_downloaded` workflow
  audit rows after successful path resolution.
- CSV/XLSX exports are generated by POST with Dolibarr token validation and add
  `export_generated` audit rows anchored to `mjlfinancement_report` rows.
