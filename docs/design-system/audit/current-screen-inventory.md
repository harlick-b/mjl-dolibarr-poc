...
| Activities | `/custom/mjlfinancement/activities.php` | Activity lifecycle, dedicated create/edit and contextual-upload states, shared scoped filters/pagination, responsive operational list, guarded review-decision states, execution fields, documents, timeline. | Direct create/edit, upload, and review-state routes recheck write/read access, active entity, scope, role, ownership, and current workflow state before loading options, consuming recovery, or exposing fields; POST guards remain authoritative. | Execution and submission/correction forms remain on the dense detail UI for later slices. |
| Expenses | `/custom/mjlfinancement/expenses.php` | Expense lifecycle, shared scoped filters/pagination and list states, responsive operational cards, evidence, guarded contextual upload, and review/final-validation/disbursement states. | Direct upload and decision-state routes recheck read access, active entity, scope, role, actor separation, and current workflow state before exposing fields; POST guards remain authoritative. | Creation, edit, and submission/correction forms remain on the dense detail UI for later slices. |
