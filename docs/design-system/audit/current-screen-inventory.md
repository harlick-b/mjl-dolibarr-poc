...
| Activities | `/custom/mjlfinancement/activities.php` | Activity lifecycle, shared scoped filters/pagination, responsive operational list, guarded review-decision states, execution fields, documents, timeline. | Direct review-state routes recheck read access, active entity, scope, role, actor separation, and current workflow state before exposing decision fields; POST guards remain authoritative. | Edit, execution, submission/correction, and document-upload forms remain on the dense detail UI for later slices. |
