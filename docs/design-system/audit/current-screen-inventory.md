...
| Activities | `/custom/mjlfinancement/activities.php` | Activity lifecycle, dedicated create/edit states, shared scoped filters/pagination, responsive operational list, guarded review-decision states, execution fields, documents, timeline. | Direct create/edit and review-state routes recheck write/read access, active entity, scope, role, ownership, and current workflow state before loading options, consuming recovery, or exposing fields; POST guards remain authoritative. | Execution, submission/correction, and document-upload forms remain on the dense detail UI for later slices. |
| Expenses | `/custom/mjlfinancement/expenses.php` | Expense lifecycle, shared scoped filters/pagination and list states, responsive operational cards, evidence, and guarded review/final-validation/disbursement states. | Direct decision-state routes recheck read access, active entity, scope, role, actor separation, and current workflow state before exposing fields; POST guards remain authoritative. | Creation, edit, submission/correction, and document-upload forms remain on the dense detail UI for later slices. |
