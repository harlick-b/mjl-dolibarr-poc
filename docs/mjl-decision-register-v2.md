...
| DEC-043 | Approve RST-014A as the guarded, disposable Phase 1 fixture infrastructure unit with fail-closed dual attestation, per-run non-Admin credentials, safe SQL channels, streaming shared-state evidence, secret scanning, and unconditional tenant teardown; RST-013A remains separately approval-gated. | APPROVED_IMPLEMENTATION_IN_REVIEW | RST-014A strategy and execution report | Explicit user approval on 2026-08-19; execution uses the tenant-active native `dol_hash(..., '0')` algorithm because forced `password_hash` is not verifiable without changing the preserved Admin's global algorithm |
