...
| `npm run test:rst002b-launcher` | Dedicated packet/launcher contracts plus a real encrypted-backup, forward-recovery, separately approved rollback-recovery, input-substitution, and lock-contention rehearsal | Temporary committed snapshot and unique disposable shared-shaped tenant; restore, one-off, Compose, custody, and lock resources proven absent; no shared mutation |
