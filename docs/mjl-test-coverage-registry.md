...
| `npm run test:rst002b-launcher` | Dedicated packet/launcher contracts plus real encrypted-backup re-entry after all 12 forward and 12 rollback DDL statements, both report-publication windows, input substitution, and lock contention | Temporary committed snapshot and unique disposable shared-shaped tenant; restore, one-off, Compose, custody, and lock resources fail-closed and proven absent; no shared mutation |
