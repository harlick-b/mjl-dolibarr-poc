...
| `npm run test:phase2` | Compatibility coverage for RST-006A workflow plus Planification, paginated Opérations scope, sanitized chronology, and command-backed fixture integration on the current exact RST-012 target | Shared before/after evidence; unique disposable tenant; whole-tenant teardown |
