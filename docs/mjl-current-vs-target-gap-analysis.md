...
| Dashboards, browsing and navigation | Financial-first Accueil, computed alerts, scoped browsing, permitted-action queues and Alertes/Rapports/exception navigation are implemented behind exact RST-012 readiness. Shared Phase 3A retains its predecessor surfaces. | The complete disposable Phase 3B gate, final aggregate, standalone verifier, scale benchmark, and RST-012 cutover rehearsals passed on September 11 with equal shared-state evidence and complete teardown. Signed human accessibility and guarded shared cutover remain pending. See `docs/mjl-phase-3b-final-validation-2026-09-11.md`. |
...
| Predecessor command compatibility | Current Phase 3B aggregate coverage includes the 52 planning/Phase 2 browser checks. | Standalone `npm run test:phase2` currently stops before browser tests because fresh activation reaches RST-012 before its RST-006A installer. Repair the compatibility alias separately; it is not Phase 3B acceptance evidence. |
