...
| No obsolete finance core | Finance schemas, loaders, routes, reports, update SQL, and tests are removed; the empty target Activity foundation is executed. | Target integer-XOF Opérations: RST-006A. |
...
| Activity assignment model | RST-002B is executed and verified: guarded primary/additional assignment history, the manual Validator module, Agent row-level reads, immediate revocation, reciprocal role guards, and the empty assignment schema are active. | Automatic creator-primary assignment and Activity creation remain RST-006A. |
...
local cutover completed under DEC-050 on 2026-09-02. RST-006A remains unapproved and
