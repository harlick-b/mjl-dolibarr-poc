...
- Budget lines are selected for production management by DPAF/Admin only.
  Production readiness requires create, edit, detail, amount validation,
  recalculation rules, audit history, filters by project/convention/activity,
  and protection against unsafe edits after expenses exist.
...
| Expense list/detail/create/workflow/upload | `/custom/mjlfinancement/expenses.php`, POST `create`, `update`, `upload`, `submit`, `correct`, `validate`, `reject` | Core workflow implemented with detail view, document upload, document state, timeline, CSRF, no-self-validation | `IN_SCOPE` | Secure download not implemented; preview planned later | Secure download route missing | Existing upload/validation guards remain domain-backed | Direct document download tests missing | Production docs state upload vs secure-download limits | Route requires `expense/read`; upload requires `expense/write` and ECM upload; validate blocks missing document | `npm run test:e2e` passed, 59/59; expense smoke passed | `llx_mjlfinancement_expense`, `llx_ecm_files`, budget lines | `PARTIAL` | Secure document download is not production-complete; preview is later |
...
| Budget lines management | `/custom/mjlfinancement/budgetlines.php` plus missing create/edit/detail actions | Read-only reference list for supervision | `IN_SCOPE` | Create, edit, detail, amount validation, filters, audit/history, and DPAF/Admin action UI | Recalculation and unsafe-edit rules after expenses need production controller enforcement | DPAF/Admin-only management guard needed for write actions; normal direct URL access is already blocked | Full management and unsafe-edit tests missing | Production docs must describe budget-line ownership and rules | `mjl_workspace_require_reference_data_access()` protects current read surface | `npm run test:e2e` passed, 59/59 for current read/route behavior | `llx_mjlfinancement_budget_line`, linked expenses | `PARTIAL` | Production management workflow and unsafe-edit rules not implemented |
...
| Dolibarr native ECM / documents | Native ECM plus MJL expense uploads | ECM is storage layer | `IN_SCOPE` | Guarded secure download missing; preview planned later | Secure download route missing; preview deferred | File access route must be MJL-guarded | Secure download tests missing | Deployment docs must cover storage/backup | Upload guarded in expense page; no public route added | `phase11-expense-workflow.spec.js` | `llx_ecm_files` and ECM directory | `PARTIAL` | Secure download is next required document capability |
